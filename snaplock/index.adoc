...
SnapLock는 SEC 17a-4, HIPAA, FINRA, CFTC 및 GDPR과 같은 규정을 준수하기 위해 데이터의 삭제, 변경 또는 이름 변경을 방지합니다. SnapLock를 사용하면 파일을 저장한 후 지정된 보존 기간 또는 무기한으로 삭제할 수 없고 쓸 수 없는 상태로 커밋하는 특수한 용도의 볼륨을 생성할 수 있습니다.

SnapLock를 사용하면 파일 및 스냅샷 복사본을 WORM 스토리지에 커밋하고 WORM 보호 데이터에 대한 보존 기간을 설정할 수 있습니다.

SnapLock 버전에는 SnapLock와 SnapLock Enterprise의 두 가지 버전이 있습니다. 두 버전 모두 모든 스토리지 유형의 NetApp 시스템에서 실행됩니다. 이러한 유연성 덕분에 SnapLock WORM 스토리지에 대한 비즈니스 요구사항에 맞는 스토리지의 유형과 양을 구입할 수 있습니다.

SnapLock WORM 스토리지는 NetApp Snapshot ™ 기술을 사용하고 NetApp SnapMirror ® 복제 소프트웨어 및 NetApp SnapVault ® 백업 기능을 데이터에 대한 백업 복구 보호를 제공하는 기본 기술로 활용할 수 있습니다.

[NOTE]
====
System Manager를 사용하면 SnapLock 라이센스 설치, 규정 준수 시계 설정, SnapLock 애그리게이트 및 볼륨 생성, SnapLock 볼륨 구성 등의 SnapLock 작업을 수행할 수 있습니다. 기타 SnapLock 작업의 경우 SnapLock API를 사용합니다.
...
SnapLock에서 지원되는 개방형 파일 프로토콜은 NFS(버전 2, 3 및 4) 및 CIFS(SMB 1.0, 2.0 및 3.0)입니다.
...
== SnapLock를 사용할 수 있는 경우

다음과 같은 상황에서 SnapLock를 사용해야 합니다.
...
== SnapLock를 사용하지 않아야 하는 경우

다음과 같은 상황이 발생하면 SnapLock를 사용하면 안 됩니다.

* SAN LUN을 사용하고 있습니다. SnapLock 볼륨에서는 SAN LUN이 지원되지 않습니다. 기존 기술을 사용하여 SAN LUN을 SnapLock 볼륨으로 이동할 수 있지만 이 작업은 지원되지 않으며 SnapLock 볼륨에서 SAN LUN을 포함하는 다른 작업은 지원되지 않습니다.
* SMTape를 사용하고 있습니다.
* 어레이 LUN에 대한 규정 준수 애그리게이트를 생성합니다. SnapLock 규정 준수 애그리게이트는 어레이 LUN을 지원하지 않습니다.
* SyncMirror 옵션을 사용하여 규정 준수 애그리게이트를 생성합니다. SnapLock 규정 준수 애그리게이트는 SyncMirror 플렉스를 지원하지 않습니다.
...
SnapLock에는 특수 하드웨어가 필요하지 않습니다. SnapLock는 Data ONTAP의 라이센스 기능으로, ONTAP 9를 지원하는 모든 AFF 및 FAS 시스템에서 지원됩니다. ONTAP Select에서도 지원됩니다. SnapLock는 소프트웨어 전용 솔루션이 아니라 통합 하드웨어 및 소프트웨어 솔루션입니다. 이러한 차이는 SEC 17a-4와 같이 통합 하드웨어 및 소프트웨어 솔루션이 필요한 엄격한 WORM 규정에 중요합니다. 자세한 내용은 SEC Interpretation: Electronic Storage of Broker - Dealer Records 를 참조하십시오.
...
* SnapLock를 구성합니다
* 재해 복구를 위해 WORM 파일을 관리합니다
* XYZ용 WORM에 파일을 커밋합니다
...
아니요
 a| 
예, ONTAP 9.8부터 시작합니다. 그러나 클라우드 관리자가 해당 데이터를 삭제할 수 있으므로 FabricPool 데이터를 퍼블릭 또는 프라이빗 클라우드로 계층화하면 SnapLock에서 더 이상 보호되지 않는다는 사실을 NetApp 어카운트 팀이 설명하는 제품 분산 요청을 개설해야 합니다.
...
FlexGroup 볼륨
 a| 
예. ONTAP 9.11.1부터 사용할 수 있지만 다음 기능은 지원되지 않습니다.
