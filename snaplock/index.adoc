...
SnapLock에는 특수 하드웨어가 필요하지 않습니다. SnapLock는 모든 AFF 및 FAS 시스템과 ONTAP Select에서 지원됩니다. SnapLock는 소프트웨어 전용 솔루션이 아니라 통합 하드웨어 및 소프트웨어 솔루션입니다. 이러한 차이는 SEC 17a-4와 같이 통합 하드웨어 및 소프트웨어 솔루션이 필요한 엄격한 WORM 규정에 중요합니다. 자세한 내용은 SEC Interpretation: Electronic Storage of Broker - Dealer Records 를 참조하십시오.
...
SnapLock 지원 범위는 ONTAP 9 릴리즈에서 개선되었습니다. 가장 중요한 확장은 다음과 같습니다.

스토리지 효율성: ONTAP 9.9.1부터 SnapLock은 데이터 컴팩션, 볼륨 간 중복제거, SnapLock 볼륨 및 애그리게이트를 위한 적응형 압축과 같은 스토리지 효율성 기능을 지원합니다. 스토리지 효율성에 대한 자세한 내용은 를 참조하십시오 link:https://docs.netapp.com/us-en/ontap/volumes/index.html["CLI를 통한 논리적 스토리지 관리 개요"].

FlexGroup 볼륨: ONTAP 9.11.1부터 SnapLock는 법적 증거 자료 보관, 이벤트 기반 보존 및 SnapVault용 SnapLock를 사용하는 경우를 제외하고 FlexGroup 볼륨을 지원합니다. ONTAP 9.12.1부터 SnapVault용 SnapLock는 FlexGroup 볼륨에서 지원됩니다.

SnapLock WORM 스토리지는 NetApp Snapshot ™ 기술을 사용하고 SnapMirror ® 복제 및 SnapVault ® 백업을 데이터 백업 복구 보호를 위한 기본 기술로 활용할 수 있습니다.
