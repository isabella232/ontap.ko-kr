...
스토리지 서비스 정의에 대한 자세한 내용은 을 참조하십시오 link:storage-service-definitions-reference.html["스토리지 서비스 정의"]
...
성능 관리에 대한 자세한 내용은 을 참조하십시오 link:../performance-admin/index.html["성능 관리"]
...
.예
다음 예에서는 1TB 크기의 스토리지 VM VS1에 대한 버킷을 생성하고 애그리게이트를 지정합니다.
