...
. 볼륨 생성:
+
'volume create-vserver vserver_name-volume volume_name-aggregate aggregate_name-size{integer[KB|MB|GB|TB|PB]} - security-style{NTFS|UNIX|MIXED} - user user_name_or_number-group group_name_or_number-junction-path junction_policy_name[-policy export_policy_name]'
