...
대부분의 경우 이는 SnapMirror 관계를 해제하고 일관성 그룹을 수정한 다음 보호를 다시 시작해야 하는 중단 프로세스입니다. ONTAP 9.13.1부터 활성 SnapMirror 관계가 있는 일관성 그룹에 볼륨을 추가하는 것은 무중단 작업입니다.
...
. 볼륨을 추가할 SnapMirror 구성을 찾습니다. 를 선택합니다 image:icon_kabob.gif["케밥 아이콘"] 그런 다음** 확장**.
