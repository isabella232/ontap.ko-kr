...
. 필요한 경우 스토리지 VM 피어 아래에서 * 피어 스토리지 VM * 을 클릭한 image:icon_kabob.gif["메뉴 옵션 아이콘"] 다음 * 피어 스토리지 VM * 을 클릭하여 스토리지 VM을 피어링할 수 있습니다.
. 볼륨 보호 * 를 클릭하여 볼륨을 보호합니다. LUN을 보호하려면 * 스토리지 > LUN * 을 클릭하고 보호할 LUN을 선택한 다음 를 클릭합니다 image:icon_protect.gif["보호 아이콘"].
+
