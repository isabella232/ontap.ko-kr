...
데이터 SVM용으로 CIFS 서버를 이미 구성한 경우 이러한 값을 'Security login domain-tunnel create' 명령으로 제공하고, 클러스터에 대한 Active Directory 도메인 컨트롤러 액세스를 위한 게이트웨이 또는 _tunnel_로 SVM을 구성하려고 합니다.
...
'-vserver'
 a| 
CIFS 서버가 구성된 SVM의 이름입니다.
 a| 

|===
CIFS 서버를 구성하지 않은 상태에서 Active Directory 도메인에 SVM 컴퓨터 계정을 생성하려는 경우 이러한 값을 "vserver active-directory create" 명령으로 제공합니다.
