...
* 모든 볼륨이 입니다 link:../volumes/move-volume-task.html["이동됨"] 또는  노드에서 소유하는 Aggregate에서.
...
* 데이터 LIF는 이미 존재했습니다 link:../networking/delete_a_lif.html["삭제됨"] 또는  노드에서.
...
* 참고: * 클러스터에서 노드를 제거하기 전에 노드에 연결된 디스크의 모든 시스템 및 사용자 데이터를 사용자가 액세스할 수 없도록 해야 합니다. 클러스터에서 노드를 제거한 후 노드를 다시 같은 클러스터에 연결해야 하는 경우 기술 지원 부서에 데이터 복원에 대한 지원을 문의하십시오.
