...
+
를 클릭합니다 `-application` 의 매개 변수입니다 `security login` 명령은 사용자 계정에 대한 액세스 방법을 지정합니다. 를 클릭합니다 `security login` https://docs.netapp.com/us-en/ontap-cli/security-login-create.html#description["Man 페이지"^] 추가 정보를 포함합니다.
