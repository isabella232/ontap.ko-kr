...
summary: ONTAP 9.11.0부터 시스템 용량 및 보안 규정 준수를 최적화하는 데 도움이 되는 시스템 관리자에 대한 정보를 볼 수 있습니다. 
---
= 시스템 최적화를 위한 통찰력 확보
:allow-uri-read: 
:icons: font
...
[role="lead"]
ONTAP 9.11.0부터 시스템 용량 및 보안 규정 준수를 최적화하는 데 도움이 되는 시스템 관리자에 대한 정보를 볼 수 있습니다. 모범 사례에 따라 이러한 통찰력은 한 페이지에 표시되어 즉시 조치를 시작하여 시스템을 최적화할 수 있습니다.
...
. (선택 사항) 페이지의 오른쪽 위 모서리에 있는 다음 단추 중 하나를 클릭하여 표시되는 통찰력을 필터링합니다.
+
** image:icon-security-filter.gif["파란색 잠금 아이콘"] 보안 관련 통찰력을 제시합니다.
** image:icon-capacity-filter.gif["파란색 디스크 아이콘"] 용량 관련 인사이트를 제시합니다.
...
== 인사이트를 위한 설정을 사용자 지정합니다

System Manager에서 통보할 통찰력을 사용자 정의할 수 있습니다.

.단계
. System Manager의 왼쪽 탐색 열에서 * Insights * 를 클릭합니다.
. 페이지의 오른쪽 위 모서리에서 을 클릭합니다 image:icon_gear.gif["설정 아이콘"].
