...
| System Manager의 시각화 | X | X | X | X | X | X 


| 용량 분석 | X | X | X | X | X | X 


| 비활성 데이터 정보 | X | X | X | X | X | X 


| Data ONTAP 7-Mode에서 전환된 볼륨 지원 | X | X | X | X | X |  


| System Manager에서 비활성 기간을 사용자 지정할 수 있습니다 | X | X | X | X | X |  


| 볼륨 레벨 활동 추적 | X | X | X | X |  |  


| 활동 추적 데이터를 CSV로 다운로드합니다 | X | X | X | X |  |  


| SVM 레벨의 활동 추적 을 참조하십시오 | X | X | X |  |  |  
...
| File System Analytics를 기본적으로 설정하는 옵션입니다 | X |  |  |  |  |  
|===
