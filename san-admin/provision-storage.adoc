...
[role="lead"]
특정 호스트에 대한 절차를 사용할 수 없는 경우 다음 단계를 수행하여 지원되는 SAN 호스트에 대한 스토리지를 프로비저닝할 수 있습니다.
...
. link:create-port-sets-binding-igroups-task.html["포트 세트를 생성하고 igroup에 바인딩합니다"] (선택 사항).
