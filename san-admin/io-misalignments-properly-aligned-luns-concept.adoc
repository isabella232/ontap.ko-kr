...
* https://["Virtual Storage Console for VMware vSphere 설치 및 관리 가이드 를 참조하십시오"^]
...
를 참조하십시오 http://["Solaris 호스트 유틸리티 설치 및 관리 가이드"^] 을 참조하십시오.
...
.관련 정보
https://["VMware vSphere, 기타 가상 환경 및 NetApp 스토리지 시스템에 대한 게스트 VM 파일 시스템 파티션/디스크 정렬"]
