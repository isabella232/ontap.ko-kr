...
* 클라이언트는 Mellanox RDMA 지원 NIC 카드 및 Mellanox OFED(MOFED) 네트워크 소프트웨어를 사용해야 합니다. 어댑터 지원에 대한 자세한 내용은 를 link:https://hwu.netapp.com/["NetApp Hardware Universe를 참조하십시오"^]참조하십시오. NFS over RDMA 지원 어댑터 설명 필드에 "RoCE"가 표시됩니다.
