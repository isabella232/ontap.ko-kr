...
* 클라우드 계층이 연결된 후에는 로컬 계층에서 분리할 수 없지만 를 사용할 수 있습니다 link:https:create-mirror-task.html["FabricPool 미러"] 로컬 계층을 다른 클라우드 계층에 연결합니다.
...
== StorageGRID 정합성 보장 제어 사용 시 추가 고려 사항
