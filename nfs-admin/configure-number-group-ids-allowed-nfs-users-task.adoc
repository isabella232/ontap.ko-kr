...
NOTE: 확장 그룹을 (`-auth-sys-extended-groups`사용하기 위한 AUTH_SYS 인증 사전 요구 사항을 이해하려면 기본 최대값인 16을 초과하는 그룹 ID를 사용합니다. https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/How_does_AUTH_SYS_Extended_Groups_change_NFS_authentication["auth_SYS 확장 그룹이 ONTAP 9에 대한 NFS 인증에 대한 변경 사항"]
