...
summary: CIFS 클라이언트 액세스를 성공적으로 테스트한 후에는 고급 CIFS 구성을 수행하거나 SAN 액세스를 추가할 수 있습니다. NFS 클라이언트 액세스를 성공적으로 테스트한 후 고급 NFS 구성을 수행하거나 SAN 액세스를 추가할 수 있습니다. 프로토콜 액세스가 완료되면 SVM의 루트 볼륨을 보호해야 합니다. 
---
= 추가 정보를 찾을 수 있는 위치
:icons: font
:imagesdir: ../media/
...
[role="lead"]
CIFS 클라이언트 액세스를 성공적으로 테스트한 후에는 고급 CIFS 구성을 수행하거나 SAN 액세스를 추가할 수 있습니다. NFS 클라이언트 액세스를 성공적으로 테스트한 후 고급 NFS 구성을 수행하거나 SAN 액세스를 추가할 수 있습니다. 프로토콜 액세스가 완료되면 SVM의 루트 볼륨을 보호해야 합니다.



== SMB 구성

다음을 사용하여 CIFS 액세스를 추가로 구성할 수 있습니다.
