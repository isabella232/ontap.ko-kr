...
| xref:../snaplock/set-retention-period-task.html[동일한 애그리게이트에서 SnapLock 및 비 SnapLock 볼륨을 생성] | ONTAP 9.10.1부터 SnapLock 및 비 SnapLock 볼륨이 동일한 aggregate에 존재할 수 있으므로, SnapLock 볼륨에 대한 별도의 SnapLock 애그리게이트를 생성할 필요가 없습니다. 


| xref:../consistency-groups/index.html[정합성 보장 그룹] | 정합성 보장 그룹에 볼륨 및 LUN을 클러스터링하여 스냅샷 복구를 위한 스토리지 볼륨의 정책을 관리하고 쓰기 순서를 지정합니다. 
...
| xref:../flexcache/supported-unsupported-features-concept.html[FlexCache용 NFSv4 지원] | FlexCache 볼륨은 NFSv4 프로토콜을 지원합니다. 마이그레이션 문서의 영향을 받는 문서입니다. 
