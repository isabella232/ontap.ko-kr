...
공격 가능성이 낮은 경우 ONTAP는 백그라운드에서 볼륨의 스냅샷을 생성합니다. APR 스냅샷은 이름(예:)으로 식별할 수 있습니다 `Anti_ransomware_backup.2022-12-20_1248`, 및 은 태그를 사용합니다 `anti-ransomware-backup`. 낮은 수준으로 유지되는 위협은 기록되지만 ONTAP는 낮은 위협에 대한 경고를 보내지 않습니다.

공격 가능성이 보통이면 ONTAP에서 위협을 평가하라는 EMS 알림을 생성합니다. 자세한 내용은 을 참조하십시오 xref:respond-abnormal-task.html[비정상적인 활동에 응답합니다].
