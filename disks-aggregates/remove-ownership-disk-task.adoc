...
* 스페어 디스크여야 합니다.
+
Aggregate에서 사용 중인 디스크에서는 소유권을 제거할 수 없습니다.
...
자동 디스크 할당을 활성화한 경우 ONTAP은 디스크에서 디스크를 제거하기 전에 소유권을 자동으로 재할당할 수 있습니다. 따라서 디스크를 제거할 때까지 자동 소유권 할당을 해제한 다음 다시 활성화합니다.
...
+
스토리지 디스크 원격 소유자 SYS1:0a.23, SYS1:0a.24, SYS1:0a.25

. 디스크가 루트 데이터 파티셔닝용으로 분할된 경우 다음 명령을 모두 입력하여 파티션에서 소유권을 제거합니다.
+
...
. 이전에 디스크 소유권 자동 할당을 해제했다면 디스크가 제거되거나 재할당된 후에 이 기능을 켜십시오.
+
