...
[role="lead"]
NFS 또는 CIFS 서버를 생성하기 전에 SVM을 위한 DNS 서비스를 구성해야 합니다. 일반적으로 DNS 이름 서버는 NFS 또는 CIFS 서버가 연결할 도메인의 Active Directory 통합 DNS 서버입니다.

Active Directory 통합 DNS 서버에는 도메인 LDAP 및 도메인 컨트롤러 서버에 대한 SRV(서비스 위치 레코드)가 포함되어 있습니다. SVM이 Active Directory LDAP 서버 및 도메인 컨트롤러를 찾을 수 없는 경우 NFS 또는 CIFS 서버 설정이 실패합니다.
...
CIFS 서버를 생성하기 전에 dns가 소스 중 하나인지 확인해야 합니다.
