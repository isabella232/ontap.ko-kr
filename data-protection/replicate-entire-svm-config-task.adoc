...
대상에서 이 절차를 완료합니다. 예를 들어, 소스 스토리지 VM에 SMB가 구성되어 있는 경우 새 보호 정책을 생성해야 하는 경우 System Manager를 사용하여 정책을 생성하고 * 보호 정책 추가 * 창에서 * ID 보존 * 옵션을 선택해야 합니다. 자세한 내용은 을 link:create-custom-replication-policy-concept.html["사용자 지정 데이터 보호 정책을 생성합니다"]참조하십시오.
...
. 필요한 경우 대상 설정을 변경하여 ID 보존을 재정의하고 네트워크 인터페이스 및 프로토콜을 포함하거나 제외하도록 선택합니다.
. 저장 * 을 클릭합니다.
