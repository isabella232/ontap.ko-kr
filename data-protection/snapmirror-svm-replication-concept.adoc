...
+
를 클릭합니다 `-identity-preserve false` 옵션은 SVM의 볼륨, 인증 및 권한 부여 구성과 에 나와 있는 프로토콜 및 이름 서비스 설정만 복제합니다 link:snapmirror-svm-replication-concept.html#configurations-replicated-in-svm-dr-relationships["SVM 재해 복구 관계에 복제된 구성"].
...
정합성 보장 그룹 비동기식 지원
 a| 
ONTAP 9.14.1부터 일관성 그룹이 있을 경우 최대 32개의 SVM 재해 복구 관계가 지원됩니다. 을 참조하십시오 link:https://docs.netapp.com/us-en/ontap/consistency-groups/protect-task.html["일관성 그룹 보호"] 및 link:https://docs.netapp.com/us-en/ontap/consistency-groups/limits.html["정합성 보장 그룹 제한"] 를 참조하십시오.
...
* MetroCluster 구성은 SVM 재해 복구 관계의 대상이 될 수 없습니다.
...
일관성 그룹
 a| 
ONTAP 9.14.1부터 지원됩니다. 자세한 내용은 을 참조하십시오 xref:../consistency-groups/protect.html[일관성 그룹 보호].
