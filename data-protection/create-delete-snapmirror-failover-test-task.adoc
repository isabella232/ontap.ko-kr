...
. 를 선택합니다 image:icon_kabob.gif["Alt = 메뉴 옵션"] 관계 소스 옆에 있는 * Test Failover * 를 선택합니다.
. Test Failover * 창에서 * Test Failover * 를 선택합니다.
. 스토리지 > 볼륨 * 을 선택하고 테스트 페일오버 볼륨이 표시되는지 확인합니다.
. 스토리지 > 공유 * 를 선택합니다.
. 을 클릭합니다 image:icon_add_blue_bg.gif["Alt = 공유 추가 메뉴"] 공유 * 를 선택합니다.
...
.단계
. 대상 클러스터에서 * 보호 > 관계 * 를 선택합니다.
. 를 선택합니다 image:icon_kabob.gif["Alt = 메뉴 옵션"] 관계 소스 옆에 있는 * Clean Up Test Failover * 를 선택합니다.
