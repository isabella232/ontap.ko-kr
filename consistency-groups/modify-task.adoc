...
* ONTAP 9.14.1부터 SnapMirror 비동기식을 사용할 때 볼륨 세분화 스냅샷을 대상 클러스터에 복제할 수 있습니다. SnapMirror 비동기식을 사용하여 정합성 보장 그룹을 확장하는 경우, 볼륨 세부 스냅샷은 SnapMirror 정책이 MirrorAll 또는 MirrorAndVault일 때 정합성 보장 그룹을 확장한 후에만 복제됩니다. 볼륨 세분화 스냅샷만 기본 일관성 그룹 스냅샷보다 최신입니다.
