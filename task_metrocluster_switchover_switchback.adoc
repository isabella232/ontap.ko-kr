...
.. 유효성 검사에 실패하고 사이트 B가 다운되면 연결 문제가 있을 가능성이 높습니다. 사이트 B가 실제로 다운되었는지 확인한 다음 의 단계를 link:https://docs.netapp.com/us-en/ontap-system-manager-classic/online-help-96-97/task_performing_unplanned_switchover.html["계획되지 않은 전환 수행"^]수행합니다.
...
. 로컬 클러스터(왼쪽에 표시)에서 를 클릭하고 image:icon_kabob.gif["메뉴 옵션 아이콘"]* 로컬 사이트로 원격 데이터 서비스 전환 * 을 선택합니다.
+
...
. 로컬 클러스터(왼쪽에 표시)에서 을 클릭하고 image:icon_kabob.gif["메뉴 옵션 아이콘"]* Take back control * 을 선택합니다.
+
데이터가 두 클러스터 간에 동기화되고 미러링되도록 하기 위해 데이터가 _DIVATED_FIRST입니다.

. 데이터 복구가 완료되면 를 클릭하고 image:icon_kabob.gif["메뉴 옵션 아이콘"]* 스위치 백 시작 * 을 선택합니다.
+
