...
스토리지 > 스토리지 VM * 을 선택한 다음 * 스토리지 VM을 선택하고 * 설정 * 을 선택합니다. 마지막으로 을 선택합니다 image:icon_pencil.gif["편집 아이콘"] S3 * 아래.
+
...
.. 를 선택합니다 image:../media/icon_arrow.gif["오른쪽 화살표"] 보호 정책 * 옆에 있는 * 추가 * 를 클릭합니다.
.. 정책 이름과 설명을 입력합니다.
