...
. 기존 사용자 및 그룹이 존재하고 소스 및 대상 스토리지 VM 모두에서 올바른 액세스 권한이 있는지 확인합니다. * 스토리지 > 스토리지 VM을 선택하고 * 스토리지 VM을 선택한 다음 * 설정 * 탭을 선택합니다. 마지막으로 * S3 * 타일을 찾아 를 선택하고 image:icon_pencil.gif["편집 아이콘"]* 사용자 * 탭을 선택한 다음 * 그룹 * 탭을 선택하여 사용자 및 그룹 액세스 설정을 확인합니다.
+
