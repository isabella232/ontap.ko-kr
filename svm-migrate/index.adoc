...
| MetroCluster SVM | ONTAP 9.16.1 | SVM을 비 MetroCluster HA 쌍에서 MetroCluster 구성으로 마이그레이션하거나 MetroCluster 구성에서 비 MetroCluster HA 쌍으로 마이그레이션할 수 있습니다. MetroCluster 구성 간에 SVM을 다른 MetroCluster 구성으로 마이그레이션할 수 없습니다. [참고] SVM 마이그레이션은 ONTAP 9.16.1 이전 릴리즈에서 MetroCluster SVM 마이그레이션을 지원하지 않습니다. 에 SnapMirror 비동기식 복제를 사용할 수 link:https://www.netapp.com/media/83785-tr-4966.pdf["MetroCluster 구성에서 SVM 마이그레이션"^]있습니다. SnapMirror 비동기식을 사용하여 MetroCluster 구성에서 SVM을 마이그레이션하는 것은 _ Disruptive_migration 방식이라는 점을 알아야 합니다. 


| NetApp 애그리게이트 암호화(NAE) | 지원되지 않습니다 | NAE를 사용하는 엔드포인트에 대해서는 마이그레이션이 지원되지 않습니다. 
