...
* 소스 및 타겟 클러스터에 SnapMirror Synchronous가 있어야 합니다 link:https://docs.netapp.com/us-en/ontap/system-admin/install-license-task.html["라이센스가 설치되었습니다"].
...
| SnapMirror Cloud 관계 | ONTAP 9.12.1 | ONTAP 9.12.1부터 SnapMirror Cloud 관계와 SVM을 마이그레이션할 때 타겟 클러스터에 SnapMirror Cloud 라이센스가 설치되어 있어야 하며, 클라우드로 미러링되는 볼륨의 용량 이동을 지원할 수 있는 충분한 용량이 있어야 합니다. 
