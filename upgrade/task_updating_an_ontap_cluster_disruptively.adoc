...
. 새 ONTAP 이미지로 노드 또는 노드 세트를 재부팅한 후 새 소프트웨어가 실행 중인지 확인합니다. 'system node image show'
+
...
+
업그레이드가 성공하지 못한 경우 노드에서 system node upgrade-revert upgrade 명령을 실행합니다. 이 명령으로 노드의 업그레이드가 완료되지 않으면 기술 지원 팀에 즉시 문의하십시오.
