...
이 콘텐츠는 AFF A-Series 및 C-Series, ASA, FAS, C-Series 시스템과 같은 NetApp 플랫폼 시스템을 지원하는 ONTAP 버전에 적용됩니다. 이 정보는 9.12.0과 같은 ONTAP 클라우드 릴리스(9.x.0)에는 적용되지 않습니다.
...
| ONTAP 9.7 이하 | 7일 
|===
* ONTAP 9.8부터 혼합 버전의 ONTAP 클러스터는 최대 4개의 주요 릴리즈를 포괄할 수 있습니다. 예를 들어 혼합 버전의 ONTAP 클러스터에는 ONTAP 9.8과 9.12.1을 실행하는 노드가 있거나 ONTAP 9.9.1 및 9.13.1 노드를 실행하는 노드가 있을 수 있습니다. ONTAP 9.8 및 9.13.1 노드를 실행하는 혼합 버전 ONTAP 클러스터는 5개의 주요 릴리스에 걸쳐 있으므로 지원되지 않습니다.
...
클러스터에 새 노드를 추가할 계획이며 이러한 노드에 현재 클러스터에서 실행 중인 버전보다 최신 버전의 ONTAP가 필요한 경우 새 노드를 추가하기 전에 클러스터의 기존 노드에서 가능한 소프트웨어 업그레이드를 수행해야 합니다.
...
. link:https://docs.netapp.com/us-en/ontap/upgrade/concept_upgrade_methods.html["업그레이드"] 새 컨트롤러에 필요한 최소 ONTAP 버전을 지원하는 클러스터의 모든 노드
. 클러스터에 새 노드를 추가합니다.
+
...
+
ONTAP 명령 클러스터를 사용합니다 `remove-node -skip-last-low-version-node-check` 이 노드를 제거합니다. (ONTAP 9.3 및 이전 버전에서는 을 사용합니다 `cluster unjoin -skip-last-low-version-node-check` 명령)
