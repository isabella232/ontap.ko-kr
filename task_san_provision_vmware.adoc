...
.시스템 관리자
--
ONTAP 9.7 이상을 위한 System Manager를 사용하여 스토리지를 제공할 LUN을 생성합니다.

LUN은 ESXi 호스트에 디스크로 표시됩니다.

System Manager Classic(ONTAP 9.7 및 이전 릴리즈의 경우)을 사용하여 ESXi 호스트에서 데이터 저장소로 SAN 프로토콜 액세스를 설정하려면 다음 항목을 참조하십시오.

* https://docs.netapp.com/us-en/ontap-sm-classic/fc-config-esxi/index.html["VSC 개요를 사용하여 ESXi용 FC 구성"^]
* https://docs.netapp.com/us-en/ontap-sm-classic/iscsi-config-esxi/index.html["VSC 개요를 사용한 ESXi용 iSCSI 구성"^]
...
.CLI를 참조하십시오
--
ONTAP CLI에서 스토리지를 제공할 LUN을 생성합니다.
