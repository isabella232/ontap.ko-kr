...
+
또는 인증, 서비스 품질 및 데이터 보호와 같은 추가 필수 서비스를 계속 활성화할 수도 있습니다. 을 참조하십시오  the volume configuration그런 다음 여기로 돌아와 다음 단계를 완료합니다.
...
볼륨을 저장한 후 로 돌아갑니다  Windows 및 Linux 서버에 대한 멀티프로토콜 프로비저닝을 완료합니다.
